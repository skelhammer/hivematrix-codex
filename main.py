...
# --- CONFIGURE LOGGING ---
# This will capture logs from your app, waitress, and apscheduler
...
def create_app():
    ...
    # Use PostgreSQL connection string from config
    app.config['SQLALCHEMY_DATABASE_URI'] = config.get('database', 'connection_string', fallback=f"sqlite:///{os.path.join(app.instance_path, 'nexus_brainhair.db')}")
    app.config['SQLALCHEMY_TRACK_MODIFICATIONS'] = False
    
    # Debug mode for Flask extensions, but Waitress controls server logging
    app.debug = '--dev' in sys.argv
    ...
    def api_billing_summary(current_user):
        ...
        try:
            ...
            logger.error(f"Error in /api/billing_summary: {e}", exc_info=True)
...
if __name__ == '__main__':
    ...
    # The scheduler should only run in the main process, not the reloader subprocess
    if not is_dev_mode or os.environ.get('WERKZEUG_RUN_MAIN') == 'true':
        with app.app_context():
            db.create_all()
    ...
    if is_dev_mode:
        ...
        if ssl_context:
            serve(app, host='0.0.0.0', port=5000, ssl_context=ssl_context)
        else:
